...
internal sealed class MainWindow : Form
{
    ...
    public MainWindow(bool smoke)
    {
        ...
        header.Controls.Add(state, 1, 0); root.Controls.Add(header, 0, 0);
        ...
        previewPanel.Controls.Add(previewHint); previewHint.BringToFront(); right.Controls.Add(previewPanel, 0, 0);
        metrics.Dock = DockStyle.Fill; metrics.Padding = new Padding(12); metrics.BackColor = Surface;
        right.Controls.Add(metrics, 0, 1); right.Controls.Add(log, 0, 2);
        middle.Controls.Add(right, 1, 0); root.Controls.Add(middle, 0, 1);
        ...
        footer.Controls.AddRange([record, stop, apply]); root.Controls.Add(footer, 0, 2); Controls.Add(root);
        FillSettings();
    ...
    private void BuildTabs()
    {
        ...
        Add(disk, ButtonText("Открыть папку журнала", (_, _) => { Directory.CreateDirectory(Storage.Root); Process.Start(new ProcessStartInfo(Storage.Root) { UseShellExecute = true }); }));
    ...
    {
        foreach (var control in controls) { control.Margin = new Padding(0, 0, 0, 12); panel.Controls.Add(control); }
    }
    ...
    private static Label LabelText(string text, float size = 10, Color? color = null) => new() { Text = text, AutoSize = false, Width = 340, Height = Math.Max(32, (text.Count(c => c == '\n') + 1) * 24 + (text.Length > 110 ? 70 : text.Length > 65 ? 28 : 0)), ForeColor = color ?? TextColor, Font = new Font("Segoe UI", size) };
    ...
    {
        address.Items.AddRange(Storage.Addresses()); address.SelectedItem = options.ListenAddress;
        ...
        bitrate.Value = Math.Clamp(options.Capture.BitrateMbps, 10, 200); buffer.Value = Math.Clamp(options.Capture.BufferMiB, 128, 4096);
        previewEnabled.Checked = options.Capture.Preview;
        exposure.Value = (decimal)Math.Clamp(options.Capture.ExposureBias, -2, 2);
        autofocus.Checked = options.Capture.Focus < 0; focus.Value = (decimal)Math.Clamp(options.Capture.Focus, 0, 1); focus.Enabled = !autofocus.Checked;
        autoWhite.Checked = options.Capture.WhiteBalanceKelvin == 0; white.Value = Math.Clamp(options.Capture.WhiteBalanceKelvin, 2000, 9000); white.Enabled = !autoWhite.Checked;
        videoOnly.Checked = options.VideoOnly; audioOffset.Value = Math.Clamp(options.AudioOffsetMs, -2000, 2000);
    ...
    private void RefreshMicrophones()
    {
        ...
        try { var devices = FifineRecorder.Devices(); microphone.Items.AddRange(devices); microphone.SelectedItem = devices.FirstOrDefault(d => d.Id == selected) ?? devices.FirstOrDefault(d => d.Name.Contains("fifine", StringComparison.OrdinalIgnoreCase)); }
    ...
    private void SaveSettings()
    {
        ...
        options.Port = (int)port.Value; options.OutputDirectory = Path.GetFullPath(output.Text);
        options.MicrophoneId = (microphone.SelectedItem as AudioDevice)?.Id ?? ""; options.VideoOnly = videoOnly.Checked;
        options.AudioOffsetMs = (int)audioOffset.Value; options.KeepSources = keepSources.Checked; options.FfmpegPath = ffmpeg.Text.Trim();
    ...
    private async Task RestartAsync()
    {
        ...
        SaveSettings();
        var host = new ReceiverHost(options, identity, audio); host.Log += AppendLog;
        try { await host.StartAsync(); receiver = host; }
        ...
        using var stream = new MemoryStream(image.GetGraphic(6)); using var original = Image.FromStream(stream);
        var old = qr.Image; qr.Image = new Bitmap(original); old?.Dispose();
        connectionInfo.Height = 58;
    ...
    private void RefreshStatus()
    {
        ...
        apply.Enabled = !operating && host?.Busy != true; tabs.Enabled = !operating && host?.Busy != true;
        ...
        {
            try { using var stream = new MemoryStream(bytes); using var image = Image.FromStream(stream); var old = preview.Image; preview.Image = new Bitmap(image); old?.Dispose(); lastPreview = bytes; previewHint.Visible = false; }
    ...
    private Task AllowFirewallAsync()
    {
        ...
        start.ArgumentList.Add("-NoProfile"); start.ArgumentList.Add("-Command"); start.ArgumentList.Add(script); Process.Start(start);
        AppendLog("Запрошено разрешение брандмауэра только для частной локальной сети."); return Task.CompletedTask;
    }
    private async Task RecoverAsync()
    {
        ...
        var file = await receiver!.RecoverAsync(dialog.SelectedPath, video == DialogResult.Yes); AppendLog("Восстановлено: " + file);
    }
    private async void ClosingAsync(object? sender, FormClosingEventArgs e)
    {
        if (closing || smoke) return;
        e.Cancel = true;
        if (receiver?.IsFinalizing == true)
        {
            MessageBox.Show(this, "Выполняется сборка MP4. Закройте приложение после её завершения.", "AirTake", MessageBoxButtons.OK, MessageBoxIcon.Information); return;
        }
        if (receiver?.Busy == true)
        {
            var answer = MessageBox.Show(this, "Дубль ещё не завершён. Прервать приём и закрыть приложение?\n\nПолученные фрагменты и WAV останутся на диске. Непереданные фрагменты останутся в iPhone. Звук после закрытия ПК не запишется. Для обычного завершения выберите «Нет» и нажмите ОСТАНОВИТЬ.", "AirTake — незавершённая запись", MessageBoxButtons.YesNo, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button2);
